...
int main(int argc, char* *argv) {
  ...
  if (rank == 0) {
    for (int i = 1; i < size; i++) {
      MPI_Recv(a+i*n_local, n_local, MPI_INT, i, 0, MPI_COMM_WORLD, NULL);
      merge(a, a, a+i*n_local, i*n_local, n_local);
    }
  } else {
    MPI_Send(a+start, n_local, MPI_INT, 0, 0, MPI_COMM_WORLD);
  }
