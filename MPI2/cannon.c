...
void Cannon(int n, GRID_INFO_T* grid, LOCAL_MATRIX_T* local_A, LOCAL_MATRIX_T* local_B, LOCAL_MATRIX_T* local_C) {
    MPI_Status status;
    int left = (grid->my_col - 1 + grid->q) % grid->q;
    int right = (grid->my_col + 1) % grid->q;
    int up = (grid->my_row - 1 + grid->q) % grid->q;
    int down = (grid->my_row + 1) % grid->q;
    
    // Initial skewing for A: shift left by my_row steps
    for (int s = 0; s < grid->my_row; s++) {
        MPI_Sendrecv_replace(local_A, 1, local_matrix_mpi_t, left, 0, right, 0, grid->row_comm, &status);
    }
    
    // Initial skewing for B: shift up by my_col steps
    for (int s = 0; s < grid->my_col; s++) {
        MPI_Sendrecv_replace(local_B, 1, local_matrix_mpi_t, up, 0, down, 0, grid->col_comm, &status);
    }
